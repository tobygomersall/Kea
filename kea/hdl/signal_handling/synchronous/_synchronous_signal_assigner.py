...
@block
def intbv_signal_assigner(clock, signal_in, signal_out, offset):
...
@block
def intbv_to_signed_signal_assigner(clock, signal_in, signal_out, offset):
...
@block
def synchronous_signal_assigner(
    clock, signal_in, signal_out, offset=0, convert_to_signed=False):
    ...
    if len(signal_out) == 1:
        return boolean_signal_assigner(clock, signal_in, signal_out)

    else:
        if convert_to_signed:
            return (
                intbv_to_signed_signal_assigner(
                    clock, signal_in, signal_out, offset))

        else:
            return intbv_signal_assigner(clock, signal_in, signal_out, offset)
